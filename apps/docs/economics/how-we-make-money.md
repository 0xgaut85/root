...
There is exactly one revenue stream: **AI labs pay the network for bandwidth, and Root Network keeps 30% of what they pay.** The other 70% goes to contributors. There is no token, no advertising, and no sale of data.

<div class="rn-split"><div class="rn-split__a">70% · paid to contributors</div><div class="rn-split__b">30% · Root</div></div>
...
3. 70% of the window's lab payments form the **contributor pool**, distributed to nodes by weighted bytes delivered.
4. 30% is retained by Root Network.

The 30% is a flat network fee. It does not vary by contributor, region or volume, and it is not layered with hidden spreads: contributors receive exactly 70% of the price labs paid.
